...
Theresa May got the most amount of money, £2809765.42.
...
```{r}
#Find the sum of money received by each MP
mp_money <- payments_db %>% 
  group_by(member_id) %>% 
  summarise(sum_money = sum(value))

#Left join the tables
left_join(mp_money, members_db,
          by = c("member_id" = "id")) %>% 
  
#Find the MP who received the most amount of money
  arrange(desc(sum_money)) %>% 
  select(member_id, name, sum_money) %>% 
  print()
```
...
The entity called Withers LLP contributed 5.25% of the total donations. They gave money to Sir Geoffrey Cox.

```{r warning=FALSE}
#Calculate the sum of donations made by each entity
pct_ent <- payments_db %>%
  group_by(entity,member_id) %>%
  summarise(sum_ent = sum(value)) %>%
  
  #Calculate the percentage of money that this entity contribute to the total donations
  ungroup() %>% 
  mutate(pct_ent = round(sum_ent/sum(sum_ent)*100,2)) %>%
  filter(pct_ent > 5) %>% 
  arrange(desc(pct_ent))
  
#Find the name of the MP
pct_ent_mp <- 
  left_join(pct_ent, members_db, 
            by = c("member_id" = "id")) %>% 
  select(entity, pct_ent, name) %>% 
  print()
```
...
```{r}
payments_db %>% 
  summarise(count_ent = n_distinct(entity))
```

-   How many (as a number and %) donated to MPs belonging to a single party only?

There were 2036 entities that only donated to MPs belonging to a single party only, accounting for 92% of all.

```{r}
#Left join the two tables
sole_party_ent <- left_join(payments_db, members_db,
                      by = c("member_id" = "id")) %>%
  
  #Group by party
  group_by(entity) %>% 
  
  #See how many distinct party did one entity donate to
  summarise(count_party = n_distinct(party_id)) %>%
  
  #Filter those entities that only donated to one party
  filter(count_party == 1) %>% 
  
  #Count how many of them there were
  summarise(
    sole_party_ent_n = sum(count_party),
    sole_party_ent_pct =
      round((sum(count_party)/2213)*100, 2)
            ) %>% 
  print()
```
...
**Step 1**: generates the table with total donations to a party in each year and its proportion in that year's total donation to all parties.

```{r}
#Get the party and donations by party data
...
donation_party <- 
  ...
  #Join the tables
  left_join(x = party_donations_db, y = parties_db,
          by = c("party_id" = "id")) %>% 
  
  #Create a new column to get the year of the donations
  mutate(year = year(date)) %>% 
  
  #Group the data by name of the party and the year of donation
  group_by(year, name) %>% 
  
  #Add up total value of donations
...
donation_party %>%
  ...
  #filter for top 10 donations for each party
  group_by(year, name) %>% 
  slice_max(total_year_donations, 
            total_year_donations = 10) %>% 
   
  #plot the table
  ggplot()+
  
  aes(x = reorder(name, -total_year_donations),
